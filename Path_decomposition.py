...
def set_substraction(node1, node2):
    ''' Returns the set (node1.value - node2.value) in the form of a list
    Parameters:
        node1 (Node object)
        node2 (Node object)
    Returns:
...
def canonical_path(parent, child):
    ...
    # we substitute this edge by the new nodes
    ...
    if len(arrival_set_minus) == 0:
        ...
        if former_node.parent:
            parent=former_node.parent
            former_node.delete_edge(parent)
...
def nice_path(path):
    ...
    while actual_node and actual_node.children != []:
        former_node = actual_node
        # there is only a child each time
        actual_node = former_node.children[0]
